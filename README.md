...
WeClaw 默认启用对话守卫：每个绑定最多连续主动下发 10 条微信消息，且接收微信必须至少每 24 小时主动给机器人发一条消息。达到限制或超过 24 小时后，程序会暂停这个绑定的通知，把微博保留为待重试，并每隔一段时间在运行日志里提醒；检测到接收人来信后会自动恢复发送。可通过 `notifications.weclaw.conversationGuard` 调整阈值和提醒间隔。
